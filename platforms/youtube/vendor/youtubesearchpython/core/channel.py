...
class ChannelCore(RequestCore):
    ...
    def parse_response(self):
        ...
        self.result = {
            ...
            "subscribersCount": {
                "text": getValue(response, ["header", "c4TabbedHeaderRenderer", "subscriberCountText", "simpleText"]),
                "short": getValue(response, ["header", "c4TabbedHeaderRenderer", "subscriberCountText", "simpleText"]),
        ...
        if not self.result["subscribersCount"]["text"]:
            self.result["subscribersCount"]["text"] = "No subscribers"
        if not self.result["subscribersCount"]["short"]:
            self.result["subscribersCount"]["short"] = self.result["subscribersCount"]["text"]
        if not self.result["subscribersCount"]["label"]:
            self.result["subscribersCount"]["label"] = self.result["subscribersCount"]["text"]
