...
class PlaylistCore(RequestCore):
        ...
        for video in videorenderer:
            try:
                ...
                j = {
                    ...
                    "title": self.__getText(self.__getValue(video, ["title"])),
                    "channel": {
                        "name": self.__getText(self.__getValue(video, ["shortBylineText"])),
                        "id": self.__getValue(video, ["shortBylineText", "runs", 0, "navigationEndpoint", "browseEndpoint", "browseId"]),
                        "link": self.__getCanonicalUrl(self.__getValue(video, ["shortBylineText", "runs", 0, "navigationEndpoint"])),
                    },
                    "duration": self.__getText(self.__getValue(video, ["lengthText"])),
                    "accessibility": {
                    ...
                    "link": self.__getCanonicalUrl(self.__getValue(video, ["navigationEndpoint"])),
                    "isPlayable": self.__getValue(video, ["isPlayable"]),
                }
                videos.append(self.__sanitizeVideo(j))
            except:
                pass

        view_count_text = self.__getText(self.__getValue(inforenderer, ["stats", 1]))
        video_count_text = self.__getText(self.__getValue(inforenderer, ["stats", 0]))
        description = self.__getText(self.__getValue(inforenderer, ["description"]))
        playlistElement = {
            'info': {
                ...
                "title": self.__getText(self.__getValue(inforenderer, ["title"])),
                "description": description,
                "videoCount": video_count_text,
                "viewCount": {
                    "text": view_count_text,
                    "short": view_count_text,
                },
                ...
                "channel": {
                    ...
                    "name": self.__getText(self.__getValue(channelrenderer, ["title"])) if channel_details_available else None,
                    "detailsAvailable": channel_details_available,
                    "link": self.__getCanonicalUrl(self.__getValue(channelrenderer, ["title", "runs", 0, "navigationEndpoint"])) if channel_details_available else None,
                    "thumbnails": self.__getValue(channelrenderer, ["thumbnail", "thumbnails"]) if channel_details_available else None,
                }
            },
            'videos': videos,
        }
        playlistElement['info'] = self.__sanitizeInfo(playlistElement['info'])
        ...
        for videoElement in continuationElements:
            if playlistVideoKey in videoElement.keys():
                videoComponent = {
                    ...
                    'title': self.__getText(self.__getValue(videoElement, [playlistVideoKey, 'title'])),
                    'thumbnails': self.__getValue(videoElement, [playlistVideoKey, 'thumbnail', 'thumbnails']),
                    'link': self.__getCanonicalUrl(self.__getValue(videoElement, [playlistVideoKey, "navigationEndpoint"])),
                    'channel': {
                        'name': self.__getText(self.__getValue(videoElement, [playlistVideoKey, 'shortBylineText'])),
                        'id': self.__getValue(videoElement,
                                              [playlistVideoKey, 'shortBylineText', 'runs', 0, 'navigationEndpoint',
                                               'browseEndpoint', 'browseId']),
                        "link": self.__getCanonicalUrl(self.__getValue(videoElement, [playlistVideoKey, "shortBylineText", "runs", 0, "navigationEndpoint"]))
                    },
                    'duration': self.__getText(self.__getValue(videoElement, [playlistVideoKey, 'lengthText'])),
                    'accessibility': {
                ...
                playlistComponent['videos'].append(
                    self.__sanitizeVideo(videoComponent)
                )
    ...
    def __getCanonicalUrl(self, endpoint: dict) -> Union[str, None]:
        if endpoint is None:
            return None
        url = self.__getValue(endpoint, ['commandMetadata', 'webCommandMetadata', 'url'])
        if isinstance(url, str):
            return self.__normalizeUrl(url)
        url = self.__getValue(endpoint, ['browseEndpoint', 'canonicalBaseUrl'])
        if isinstance(url, str):
            return self.__normalizeUrl(url)
        return None
    ...
    def __defaultText(self, value, fallback):
        if isinstance(value, str) and value.strip():
            return value
        return fallback

    def __sanitizeChannel(self, channel):
        channel['id'] = self.__defaultText(channel.get('id'), 'No channel id')
        channel['name'] = self.__defaultText(channel.get('name'), 'No channel')
        channel['link'] = self.__defaultText(self.__normalizeUrl(channel.get('link')), 'No channel link')
        channel['thumbnails'] = channel.get('thumbnails') or []
        return channel

    def __sanitizeVideo(self, video):
        video['id'] = self.__defaultText(video.get('id'), 'No id')
        video['title'] = self.__defaultText(video.get('title'), 'No title')
        video['thumbnails'] = video.get('thumbnails') or []
        video['link'] = self.__defaultText(self.__normalizeUrl(video.get('link')), 'No link')
        video['duration'] = self.__defaultText(video.get('duration'), 'No duration')
        video['channel'] = self.__sanitizeChannel(video.get('channel') or {})
        return video

    def __sanitizeInfo(self, info):
        info['id'] = self.__defaultText(info.get('id'), 'No id')
        info['title'] = self.__defaultText(info.get('title'), 'No title')
        info['description'] = self.__defaultText(info.get('description'), 'No description')
        info['videoCount'] = self.__defaultText(info.get('videoCount'), 'No videos')
        view_text = self.__defaultText((info.get('viewCount') or {}).get('text') if isinstance(info.get('viewCount'), dict) else None, 'No views')
        info['viewCount'] = {'text': view_text, 'short': view_text}
        info['thumbnails'] = info.get('thumbnails') or []
        info['link'] = self.__defaultText(self.__normalizeUrl(info.get('link')), 'No link')
        info['channel'] = self.__sanitizeChannel(info.get('channel') or {})
        return info
