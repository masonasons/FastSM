...
class audio(wx.Panel, wx.Dialog):
	def __init__(self, parent):
		...
		try:
			...
			# If we can't enumerate devices, add a default
			import logging
			logging.warning(f"Could not get audio devices in options: {e}")

		# Ensure there's at least one device option
		if not self.device_names:
			self.device_names.append("Default audio device")
			self.device_indices.append(1)
